...
def parse_package_metadata(data, mirror_url, snapshot, package_prefix):
        ...
        if line:
            ...
            if re.match(r"\s", line):
                ...
                if current_key in current_entry:
                    raise Exception(
                        "Duplicate key for package metadata:" + current_key + "\n" + current_entry
                    )
    ...
    for pkg_data in parsed_entries:
        if package_prefix:
            parsed_entries[pkg_data] = package_prefix + parsed_entries[pkg_data][FILENAME_KEY]
        else:
            parsed_entries[pkg_data] = (
                mirror_url + "/debian/" + snapshot + "/" + parsed_entries[pkg_data][FILENAME_KEY]
            )
