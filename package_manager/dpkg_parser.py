...
def main():
    ...
    if args.download_and_extract_only:
        download_package_list(
            ...
            args.snapshot,
            args.sha256,
            args.packages_gz_url,
            args.package_prefix,
        )
        util.build_os_release_tar(
            args.distro, OS_RELEASE_FILE_NAME, OS_RELEASE_PATH, OS_RELEASE_TAR_FILE_NAME
        )
    else:
    ...
    for pkg_vals in set(packages.split(",")):
        ...
        for package_file in package_files.split(","):
            ...
            if pkg_name in metadata and (
                pkg_version == "" or pkg_version == metadata[pkg_name][VERSION_KEY]
            ):
                ...
                package_to_rule_map[pkg_name] = package_to_rule(workspace_name, pkg_name)
                ...
                if actual_checksum != expected_checksum:
                    raise Exception(
                        ...
                        % (pkg_name, pkg[FILENAME_KEY], expected_checksum, actual_checksum)
                    )
                if pkg_version == "":
                    break
                if (
                    pkg_vals in pkg_vals_to_package_file_and_sha256
...
def download_and_save(pkg_key, url, out_file, retry_count=20):
    ...
    if remaining_bytes != 0:
        ...
        if not range_access_enabled:
            raise Exception(
                "Fail to download %s (%s). Server returned partial contents." % (pkg_key, url)
            )

        while retry_count > 0:
            retry_count -= 1
            req = Request(url, headers={"Range": "bytes=%d-" % offset, "If-Range": etag})
            res = urlopen(req)
            if res.getcode() != 206:
                raise Exception(
                    "Fail to download %s (%s). Server did not return '206 Partial Content'"
                    % (pkg_key, url)
                )
            downloaded = res.read()
            contents.append(downloaded)
        ...
        if remaining_bytes != 0:
            raise Exception(
                "Fail to download %s (%s). Too many Range request retries." % (pkg_key, url)
            )
...
def download_package_list(
    mirror_url, distro, arch, snapshot, sha256, packages_gz_url, package_prefix
):
...
SHA256: 52ec3ac93cf8ba038fbcefe1e78f26ca1d59356cdc95e60f987c3f52b3f5e7ef
    ...
    if sha256 != actual_sha256:
        raise Exception(
            "sha256 of Packages.gz don't match: Expected: %s, Actual:%s" % (sha256, actual_sha256)
        )
